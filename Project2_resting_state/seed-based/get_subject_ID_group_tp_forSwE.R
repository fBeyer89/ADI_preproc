library(plyr)

files=read.table("/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/scans_PCC_CC_z.txt")

for (i in 1:nrow(files)){
  #print(strsplit(toString(rs_QA[i,1]),'_')[[1]][1])
  tmp=strsplit(toString(files[i,1]),'/')[[1]][8]
  files[i,"tp"]=strsplit(toString(tmp),'_')[[1]][2]
  files[i,"subj.ID"]=strsplit(toString(tmp),'_')[[1]][1]
}

tmp2=files
#load group info
info_file=read.csv("/data/p_02161/ADI_studie/metadata/final_sample_MRI_QA_info.csv")
group=info_file[,c("subj.ID","condition","tp","Age_BL","Sex","meanFD")]
condition=merge(files, group[!is.na(group$condition),], by=c("subj.ID","tp"))

final=join(tmp2,condition)
#here: "IG" "KG"
final[final$condition=="IG","IG"]=1
final[is.na(final$IG),"IG"]=0
final[final$condition=="KG","KG"]=1
final[is.na(final$KG),"KG"]=0

final$tp=as.factor(final$tp)
levels(final$tp)=c(1,2,3)
final$Sex=as.factor(final$Sex)
levels(final$Sex)=c(0,1)

write.table(final$tp, col.names=FALSE, row.names=FALSE,quote=FALSE,
            file='/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/tp.txt')
write.table(final$Age_BL, col.names=FALSE, row.names=FALSE,quote=FALSE,
            file='/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/Age.txt')
write.table(final$Sex, col.names=FALSE, row.names=FALSE,quote=FALSE,
            file='/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/Sex.txt')
write.table(log10(final$meanFD), col.names=FALSE, row.names=FALSE,quote=FALSE,
            file='/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/logmeanFD.txt')
write.table(final$subj.ID, col.names=FALSE, row.names=FALSE,
            file='/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/subjID.txt')

##Now for covariates.
write.table(final$IG, col.names=FALSE, 
            row.names=FALSE,
            quote=FALSE,
            file='/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/group_IG.txt')
write.table(final$KG, col.names=FALSE, 
            row.names=FALSE,
            quote=FALSE,
            file='/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/group_KG.txt')

##reverse again for time covariates -> linear modeling of time.
final$tp_cov=final$tp
levels(final$tp_cov)=c(-1,0,1)

final[final$KG==1,"tp_cov"]=0
write.table(final$tp_cov, col.names=FALSE, row.names=FALSE,quote=FALSE,
            file='/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/tp_IG.txt')
final[,"tp_cov"]=final$tp
levels(final$tp_cov)=c(-1,0,1)
final[final$IG==1,"tp_cov"]=0
write.table(final$tp_cov, col.names=FALSE, row.names=FALSE,quote=FALSE,
            file='/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/tp_KG.txt')

######################################
#Different model with time as factor
######################################
final[final$tp=="3"&final$IG==1,"IG_fu2"]=1
final[final$tp=="2"&final$IG==1,"IG_fu"]=1
final[final$tp=="1"&final$IG==1,"IG_bl"]=1
final[final$tp=="3"&final$KG==1,"KG_fu2"]=1
final[final$tp=="2"&final$KG==1,"KG_fu"]=1
final[final$tp=="1"&final$KG==1,"KG_bl"]=1
final[is.na(final$IG_fu2),"IG_fu2"]=0
final[is.na(final$IG_fu),"IG_fu"]=0
final[is.na(final$IG_bl),"IG_bl"]=0
final[is.na(final$KG_fu2),"KG_fu2"]=0
final[is.na(final$KG_fu),"KG_fu"]=0
final[is.na(final$KG_bl),"KG_bl"]=0
write.table(final$IG_fu2, col.names=FALSE, row.names=FALSE,quote=FALSE,
            file='/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/IG_fu2.txt')
write.table(final$IG_fu, col.names=FALSE, row.names=FALSE,quote=FALSE,
            file='/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/IG_fu.txt')
write.table(final$IG_bl, col.names=FALSE, row.names=FALSE,quote=FALSE,
            file='/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/IG_bl.txt')
write.table(final$KG_fu2, col.names=FALSE, row.names=FALSE,quote=FALSE,
            file='/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/KG_fu2.txt')
write.table(final$KG_fu, col.names=FALSE, row.names=FALSE,quote=FALSE,
            file='/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/KG_fu.txt')
write.table(final$KG_bl, col.names=FALSE, row.names=FALSE,quote=FALSE,
            file='/data/pt_02161/Results/Project2_resting_state/connectivity/Analysis/Files/KG_bl.txt')


