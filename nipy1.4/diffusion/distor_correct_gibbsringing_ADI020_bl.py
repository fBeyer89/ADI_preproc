...
def create_distortion_correct():
    ...
    #remove last slice from B0 file.
    remove_slice=Node(fsl.ExtractROI(x_min=0, x_size=-1,
                               y_min=0, y_size=-1,
                               z_min=0, z_size=58,
                               t_min=0,  t_size=-1),    name='remove_slice')
    ...
    (inputnode, remove_slice, [('dwi_b0', 'in_file')]),
    (remove_slice, merge_list, [('roi_file', 'dwi_b0')]),
    ...
    #cut three slices of fieldmap
    remove_slice_fm=Node(fsl.ExtractROI(x_min=0, x_size=-1,
                               y_min=0, y_size=-1,
                               z_min=0, z_size=58,
                               t_min=0,  t_size=-1),    name='remove_slice_fm')  

    #cut three slices of fieldmap phase
    remove_slice_fmp=Node(fsl.ExtractROI(x_min=0, x_size=-1,
                               y_min=0, y_size=-1,
                               z_min=0, z_size=58,
                               t_min=0,  t_size=-1),    name='remove_slice_fmp')      
    
    distor_correct.connect([
    (inputnode, remove_slice_fm, [('dwi_mag', 'in_file')]),
    (remove_slice_fm, mag2b0,[('roi_file', 'in_file')]),
    ...
    distor_correct.connect(remove_slice_fm,'roi_file', bet_mag,'in_file')   
    ...
    distor_correct.connect([(erode, prep_fmap, [('out_file', 'in_magnitude')]),   
                        (inputnode, prep_fmap, [('te_diff', 'delta_TE')]),
                        (inputnode, remove_slice_fmp,[('dwi_ph','in_file')]),
                        (remove_slice_fmp, prep_fmap, [('roi_file', 'in_phase')]),
