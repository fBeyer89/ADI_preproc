...
out_dir = '/nobackup/aventurin4/data_fbeyer/multiatlas/find_best_subjects/results/labelfusionout/staple_nonbin/'


staple_wf = Workflow(name='staple_wf_nonbin_c1')
...
datasource.iterables=[('hemi',['lh','rh']), ('target_id', ['LI00145091'])] #allsubjects)]
...
binarize = Node(fsl.maths.MathsCommand(args='-bin'),
                                            name='binarize')
maths_nonbin = Node(fsl.MultiImageMaths(op_string="-mul %s -thr 0.4"), name="maths")
...
#staple.inputs.kernel_size=1.5
#for STEPS input
staple.iterables=[('template_num', [10,43]),('kernel_size', [1.5,3])]
...
(binarize, maths_nonbin, [('out_file', 'in_file')]),
#(binarize, maths, [('out_file', 'in_file')]),
#(datasource, maths, [('c1','operand_files')]),
#(maths, staple, [('out_file', 'in_file')]), 
(datasource, maths_nonbin,[('c1','operand_files')]),
(maths_nonbin, staple, [('out_file', 'in_file')]), 
